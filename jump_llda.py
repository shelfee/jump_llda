...
def llda_cvb0(gamma, t_vocab, w_vocab, alpha, eta, count):
    ...
    for c in xrange(1, count + 1):
        ...
        for d in gamma:
            ...
            for w, ts in gamma[d]:
                ...
                for t in ts:
                    g = (m0[t] + alpha_l) * (n1[t][w] + eta) / (n1_all[t] + veta)
                    ts[t] = g
                    g_sum += g
    ...
    for d in gamma:
        ...
        m0 = multiple(r_trans, n0)
        for t in theta[d]:
            theta[d][t] = (m0[t] + alpha_l) / (n0_all + len(theta[d]) * alpha_l)
...
def main():
    ...
    for i in xrange(10):
        ...
        print "fold-%02d:\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f" % (
        i, r5, p5, h5, r10, p10, h10, r20, p20, h20)
        print >> f_report, "%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f" % (
        r5, p5, h5, r10, p10, h10, r20, p20, h20)
    ...
    print "r5: %.5f, p5: %.5f, r10: %.5f, p10: %.5f, h5: %.5f, h10: %.5f \t%.5f\t%.5f\t%.5f" % (
    ar5, ap5, ar10, ap10, ah5, ah10, ar20, ap20, ah20)
    print "-------avg-------"
    print "avg time", avg_train, avg_predict

    print >> f_report, "---avg---"
    print >> f_report, "%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f" % (
    ar5, ap5, ah5, ar10, ap10, ah10, ar20, ap20, ah20)
